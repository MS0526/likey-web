...
3. 이유는 반드시 그 물품 자신의 필드 값만 그대로 옮겨서 써 — 다른 물품이나 다른 카테고리 얘기를 섞지 마.
   - isNewCategory가 true인 경우에만 "이 카테고리의 요청이 없었습니다" 같은 취지로 써. false면 그렇게 쓰면 안 돼.
   - popularityRank가 숫자인 경우에만 "N위"라고 정확히 그 숫자를 인용해서 인기 물품이라고 써. null이면 인기 순위를 언급하지 마.
   - 두 필드 다 해당 안 되면(isNewCategory false, popularityRank null) "요청 목록에 추가해볼 만한 물품입니다"처럼 담백하게만 써.
4. 문장은 반드시 "-습니다/-입니다"로 끝나는 완전한 존댓말 문장으로 써. "-다", "-음", "-됨"처럼 끊어 쓰는 개조식 말투는 쓰지 마.
   예) "이 카테고리 요청이 없었다" (X) → "이 카테고리의 요청이 없었습니다" (O)
5. 같은 문장을 여러 추천에 복사해서 쓰지 마 — 물품마다 필드 값이 다르면 이유 문장도 달라야 해.
6. 추천할 만한 후보가 정말 없으면 recommendations를 빈 배열로 반환해.
